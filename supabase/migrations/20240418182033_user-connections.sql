...
add table web_rtc_signals;

alter publication supabase_realtime
add table pairing_code_redemptions;

alter publication supabase_realtime
add table user_connections;

-- RLS for web_rtc_signals
alter table web_rtc_signals enable row level security;

create policy select_if_from_or_to_user on web_rtc_signals for
select
  using (
...
create policy insert_if_code_redemption_exists on web_rtc_signals for insert
with
  check (
    auth.uid () = from_user_id
...
create policy insert_if_user_connection_exists on web_rtc_signals for insert
with
  check (
    auth.uid () = from_user_id
    and (
...
create policy delete_if_from_user on web_rtc_signals for delete using (auth.uid () = from_user_id);

-- RLS for pairing_codes
alter table pairing_codes enable row level security;

create policy select_own on pairing_codes for
select
...
create policy select_if_own_redemption_exists on pairing_codes for
select
  using (select_if_own_redemption_exists (code));

create policy delete_own on pairing_codes for delete using (auth.uid () = user_id);

-- RLS for pairing_code_redemptions
alter table pairing_code_redemptions enable row level security;

create policy select_own on pairing_code_redemptions for
select
  using (auth.uid () = user_id);
...
create policy select_for_own_pairing_code on pairing_code_redemptions for
select
  using (user_has_pairing_code (pairing_code));

-- RLS for user_connections
alter table user_connections enable row level security;

create policy select_own on user_connections for
select
  using (
    auth.uid () = user_1_id
...
create policy insert_if_matches_pairing_code_redemption on user_connections for insert
with
  check (
    exists (
      select
...
create policy delete_own on user_connections for delete using (
  auth.uid () = user_1_id
  or auth.uid () = user_2_id
);
