create policy select_if_user_connection on users for
select
  using (
    exists (
      select
...
create table colors (id text primary key, label text not null);

alter table colors enable row level security;

create policy select_all on colors for
select
  using (true);
...
create policy select_all on animals for
select
  using (true);

insert into
  animals (id, label, emoji)
values
  ('bear', 'Bear', '🐻'),
  ('bee', 'Bee', '🐝'),
  ('bird', 'Bird', '🐦'),
...
create or replace function user_created () returns trigger as $$
begin
